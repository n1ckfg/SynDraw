...
class normalnet(object):
    ...
    def build_model(self):
        ...
        self.fake_B = self.generator(tf.concat([self.real_A, self.mask], axis=3))
        ...
        self.stacked_SAB = tf.concat([self.real_A, self.fake_B, self.real_B], 2)
        self.real_SAB_sum = tf.summary.image("arecord", self.stacked_SAB)
    ...
    def load_random_samples(self):
        data = np.random.choice(glob('./{}/val/*.png'.format(self.dataset_name)), self.batch_size)
    ...
    def generator(self, image, isSampling=False):
        with tf.variable_scope("generator") as scope:
            ...
            d3 = tf.nn.dropout(self.g_bn_d3(self.d3), 0.5) # dropout generator
    ...
    def train(self, args):
        ...
        self.g_summary = tf.summary.merge([self.real_SAB_sum, self.d_loss_fake_sum, self.g_loss_sum])
        ...
        self.visual_loss_summary = self.pixel_wised_loss_sum
        self.writer = tf.summary.FileWriter("./network/{}/logs".format(self.dataset_name), self.sess.graph)
        ...
        for epoch in range(args.epoch):
            data = glob('./{}/train/*.png'.format(self.dataset_name))
            ...
            for idx in range(0, batch_idxs):
                errD=.0
                batch_list = [self.load_training_imgs(data, idx+i) for i in range(self.n_critic)]
    ...
    def test(self, args):
        ...
        test_files = glob('{}/*.png'.format(args.test_dir))

        n = [i for i in map(lambda x: x.split('/')[-1].split('.png')[0], test_files)]
    ...
    def load_data(self, image_path, is_test=False):
        ...
        if not is_test:
            ...
            mask = np.zeros([256,256]).astype(np.float)
            img_A = input_img[0:256, 0:256, :]
            img_B = np.ones([256,256,3]).astype(np.float)            

        mask = np.reshape(mask, (256, 256, 1)) / 255.0
        img_A = img_A / 127.5 - 1.
        img_B = img_B / 127.5 - 1.
        print(img_A.shape, mask.shape, img_B.shape)
